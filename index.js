...
const main = async () => {
  ...
  const customerSarah = await models.customers.findOne({
  ...
  let numOfOrdersCust1 = await models.orders.count({
    where: { customer_id: 1 },
  });
  ...
  console.log(
    `The total price of order with ID = 1 is ${total_price_order_1}.`
  );
  ...
  const q10result = await models.products.findAll({
    attributes: [
      ...
      "price",
      [
        sequelize.fn("COUNT", sequelize.col("order_items.order_id")),
        "orderCount",
      ],
    ],
    include: [
  ...
  const q11result = await models.customers.findAll({
    attributes: [
      ...
      "last_name",
      [
        Sequelize.fn("SUM", Sequelize.col("orders.order_items.price")),
        "total_value",
      ],
    ],
    include: [
  ...
  const q12result = await models.orders.findAll({
    attributes: [
      ...
      "order_date",
      [
        Sequelize.fn("SUM", Sequelize.literal("(order_items.price > 90)")),
        "product_count",
      ],
    ],
    include: {
